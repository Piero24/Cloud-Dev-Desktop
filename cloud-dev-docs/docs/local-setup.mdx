...
**How it works:** When you first run `ssh -p 3434 abc@<server-ip>`, you enter your password as usual. SSH keeps that connection open in the background. Every subsequent `ssh`, `scp`, or `rsync` to the same server rides the existing connection — no password prompt — for up to 16 hours.
