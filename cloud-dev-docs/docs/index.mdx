...
Cloud Dev Stack is a three-container Docker Compose stack for CasaOS that gives you a complete cloud development environment.

## What You Get

- **Browser-accessible Ubuntu desktop** — full XFCE desktop from any device
- **Browser-accessible VS Code** — on a separate port
- **Landing page** — unified entry point at `general.yourdomain.com` with Desktop / Code buttons, served directly by NPM (no extra container)
...
Internet
   |
Nginx Proxy Manager (SSL termination, routes traffic + serves landing page)
   |
   +-- Authelia (user+pass → TOTP from authenticator app → session cookie)
   |     |
   |     +-- general.yourdomain.com --> [static HTML from /data/landing]  (landing page, served by NPM)
