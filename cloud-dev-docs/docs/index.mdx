...
All web access is protected by your existing Cloudflare Zero Trust 2FA. SSH is protected by password + 2FA at session start, then stays open via SSH multiplexing.
...
Internet
   |
Cloudflare Zero Trust (2FA gate — asks for OTP before anything loads)
   |
Nginx Proxy Manager (your existing NPM, routes traffic inside Docker network)
   |
   +-- desktop.yourdomain.com --> dev-desktop:3000  (Ubuntu XFCE in browser)
   +-- code.yourdomain.com    --> dev-vscode:8080   (VS Code in browser)

SSH (bypasses Cloudflare, goes direct):
   Mac --ssh -p 3434--> dev-desktop:22  (password + 2FA once per session)
                         |
