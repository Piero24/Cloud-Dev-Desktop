...
- **Everything shared**: both containers mount the same `/config` (home) and `/projects`. Install a package, set a git config, or save a file and it's visible everywhere instantly
...
- **[Local Setup](./local-setup)**: optional: edit locally, auto sync to server
