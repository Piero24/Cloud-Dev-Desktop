...
- **VS Code (desktop app)**: inside the web desktop — launch from the application menu or run `code /projects` in the desktop terminal. Shares extensions and settings with both code-server instances
...
| VS Code — desktop app | Inside the web desktop → app menu, or `code /projects` | (same desktop session) |
